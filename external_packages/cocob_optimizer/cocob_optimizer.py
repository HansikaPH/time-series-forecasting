...
class COCOB(Optimizer):
    ...
    def _apply_dense(self, grad, var):
        ...
        L_update = tf.math.maximum(L, tf.math.abs(grad))
        gradients_sum_update = gradients_sum + grad
        grad_norm_sum_update = grad_norm_sum + tf.math.abs(grad)
        reward_update = tf.math.maximum(reward - grad * tilde_w, 0)
        new_w = -gradients_sum_update / (
                    L_update * (tf.math.maximum(grad_norm_sum_update + L_update, self._alpha * L_update))) * (
