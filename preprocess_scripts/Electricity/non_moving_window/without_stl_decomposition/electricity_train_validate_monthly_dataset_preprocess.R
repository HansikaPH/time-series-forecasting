...
for (validation in c(TRUE, FALSE)) {
	OUTPUT_PATH = paste(OUTPUT_DIR, "electricity_12", sep = '/')
	if (validation) {
    ...
    for (idr in 1 : length(electricity_dataset)) {
        ...
        sav_df[, 2 : (time_series_length - max_forecast_horizon + 1)] = time_series_log[1 : (time_series_length - max_forecast_horizon)]

        sav_df[, (2 + time_series_length - max_forecast_horizon)] = '|o'

        sav_df[, (3 + time_series_length - max_forecast_horizon) : (2 + time_series_length)] = time_series_log[(length(time_series_log) - max_forecast_horizon + 1) : length(time_series_log)]
