...
for (idr in 1: nrow(numeric_dataset_log)) {
  ...
  level=time_series_log[time_series_length] #last "trend" point in the input window is the "level" (the value used for the normalization)
  ...
  # sav_df = cbind(sav_df, t(time_series_log[(time_series_length - max_forecast_horizon + 1) : time_series_length,1]))
