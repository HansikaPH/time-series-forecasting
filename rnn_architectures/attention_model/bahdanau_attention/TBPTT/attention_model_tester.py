...
# TODO: halfway through modifying
class AttentionModelTester:
    ...
    def test_model(self, **kwargs):
        ...
        with tf.variable_scope('encode'):
            training_encoder_outputs, training_encoder_final_state = tf.nn.dynamic_rnn(cell=multi_layered_encoder_cell,
                                                                                       initial_state=encoder_initial_state_tuple,
                                                                                       inputs=training_input,
                                                                                       sequence_length=input_sequence_length,
                                                                                       dtype=tf.float32)
        with tf.variable_scope('encode', reuse=tf.AUTO_REUSE):
            inference_encoder_outputs, inference_encoder_final_state = tf.nn.dynamic_rnn(
                cell=multi_layered_encoder_cell,
                initial_state=encoder_initial_state_tuple,
        ...
        with tf.variable_scope('decode'):
            ...
            decoder_initial_state = decoder_cell.zero_state(batch_size=tf.shape(training_input)[0],
                                                                              dtype=tf.float32).clone(cell_state=training_encoder_final_state)

            helper = tf.contrib.seq2seq.ScheduledOutputTrainingHelper(inputs=target,
        ...
        with tf.variable_scope('decode', reuse=tf.AUTO_REUSE):
            ...
            # create the initial state for the decoder
            decoder_initial_state = decoder_cell.zero_state(batch_size=tf.shape(training_input)[0],
                                                            dtype=tf.float32).clone(cell_state=inference_encoder_final_state)
            ...
            for epoch in range(int(max_num_epochs)):
                ...
                while True:
                    try:
                        ...
                        current_minibatch_size = np.shape(next_training_batch_value[1])[0]

                        # number_of_chunks = np.ceil(np.shape(next_training_batch_value[1])[1] / tbptt_chunk_length)
                        ...
                        encoder_initial_state_value = np.zeros(
                            ...
                            dtype=np.float32)
                        number_of_tbptt_chunks = np.shape(next_training_batch_value[1])[1]

                        # loop through the truncated mini batches for each mini-batch
                        for i in range(number_of_tbptt_chunks):
                            length_comparison_array = np.greater([i + 1] * np.shape(next_training_batch_value[1])[0],
                            ...
                            encoder_initial_state_value, _ = session.run([training_encoder_final_state, optimizer],
                                                                         feed_dict={training_input: np.expand_dims(
                                                                             training_data_batch_value[1][:, i, :],
                                                                             axis=2),
                                                                                    target: np.expand_dims(
                                                                                        training_data_batch_value[2][:,
                                                                                        i, :], axis=2),
                                                                                    encoder_initial_state_tuple: encoder_initial_state_value,
            ...
            while True:
                try:
                    ...
                    test_output = session.run(inference_decoder_outputs[0],
                                              feed_dict={training_input: test_input_batch_value[1],
                                                         ...
                                                         output_sequence_length: [self.__output_size] *
                                                                                 np.shape(test_input_batch_value[1])[0]
                                                         })
