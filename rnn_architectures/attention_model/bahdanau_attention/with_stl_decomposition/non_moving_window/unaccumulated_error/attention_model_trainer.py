...
class AttentionModelTrainer:
    ...
    def train_model(self, **kwargs):
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=gpu_configs.log_device_placement, allow_soft_placement=True, gpu_options=gpu_options)) as session:
            session.run(init_op)
            ...
            for epoch in range(max_num_epochs):
                ...
                while True:
                    try:
                        ...
                        decoder_input_value = np.hstack((np.expand_dims(training_data_batch_value[1][:, -1, :], axis=1),
                                                         training_data_batch_value[2][:, :-1, :]))
