...
class AttentionModelTrainer:
    ...
    def __init__(self, **kwargs):
        ...
        self.__seasonality_periods = kwargs["seasonality_periods"]

    def __l1_loss(self, z, t):
        loss = tf.reduce_mean(tf.abs(t - z))
        return loss

    # TODO: whether take the encoder outputs directly, which weights to assign to each timestep?, only for single seasonality?
    def custom_attention(self, encoder_outputs, current_timestep, sequence_length):
        weight = 1 / len(self.__seasonality_periods)
        decoder_input = []

        for period in self.__seasonality_periods:
            if current_timestep - period >= 0:
                decoder_input = decoder_input + encoder_outputs[current_timestep - period] * weight

        tf.layers.conv1d(encoder_outputs[:, 0:sequence_length])
    ...
    def train_model(self, **kwargs):
        ...
        decoder_initial_state = multi_layered_decoder_cell.zero_state(batch_size = tf.shape(input)[0], dtype = tf.float32).clone(cell_state = encoder_state)
