...
class AttentionModelTester:
    ...
    def test_model(self, **kwargs):
        ...
        time_series_wise_attention_features_beginning_indices = input_sequence_length - seasonality_period
        time_series_wise_attention_features_ending_indices = input_sequence_length

        # building the decoder network for training
        with tf.variable_scope('decoder_train_scope') as decoder_train_scope:
            row_indices = tf.expand_dims(tf.transpose(tf.range(0, tf.shape(time_series_wise_attention_features_beginning_indices)[0])), axis=1)

            attention_features_beginning_indices = tf.concat([row_indices, tf.expand_dims(time_series_wise_attention_features_beginning_indices, axis=1)], axis=1)
            attention_features_ending_indices = tf.concat([row_indices, tf.expand_dims(time_series_wise_attention_features_ending_indices, axis=1)], axis=1)
            # training_attention_features = tf.strided_slice(training_encoder_outputs, attention_features_beginning_indices, attention_features_ending_indices)
            # array_first_dimension = tf.convert_to_tensor(tf.range(0, tf.shape(training_encoder_outputs)[0]))
            # array_last_dimension = tf.convert_to_tensor(tf.range(0, int(lstm_cell_dimension)))
            # training_attention_features = training_encoder_outputs[array_first_dimension, attention_features_beginning_indices : attention_features_ending_indices, array_last_dimension]
            training_helper = tf.contrib.seq2seq.ScheduledOutputTrainingHelper(inputs=decoder_input,
                                                                               sequence_length=output_sequence_length,
                                                                               # auxiliary_inputs = training_attention_features,
        ...
        with tf.variable_scope(decoder_train_scope, reuse=tf.AUTO_REUSE) as decoder_inference_scope:
            ...
            # inference_decoder_initial_state = multi_layered_decoder_cell.zero_state(batch_size=tf.shape(input)[0],
            #                                                                     dtype=tf.float32).clone(
            #     cell_state=training_encoder_state)

            # array_first_dimension = tf.convert_to_tensor(range(0, tf.shape(training_encoder_outputs)[0]))
            # inference_attention_features = inference_encoder_outputs[array_first_dimension, attention_features_beginning_indices: attention_features_ending_indices, :]
            # inference_attention_features = tf.strided_slice(inference_encoder_outputs,
            #                                                attention_features_beginning_indices,
            #                                                attention_features_ending_indices)
            inference_helper = tf.contrib.seq2seq.ScheduledOutputTrainingHelper(inputs=decoder_input,
                                                                                sequence_length=output_sequence_length,
                                                                                # auxiliary_inputs=inference_attention_features,
            ...
            for epoch in range(int(max_num_epochs)):
                ...
                while True:
                    try:
                        ...
                        x, y = session.run([attention_features_beginning_indices, row_indices], feed_dict={input: next_training_batch_value[1],
                                               training_target: next_training_batch_value[2],

                                               input_sequence_length: next_training_batch_value[0],
                                               })
                        # y = session.run(row_indices)
