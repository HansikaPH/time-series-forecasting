...
errors_file_full_name = paste("results/errors", errors_file_name, sep='/')
actual_results=read.csv(file=actual_results_file_name,sep=';',header = FALSE)

text_test_df=read.csv(file=txt_test_file_name,sep = " ",header = FALSE)
...
forecasts_df=read.csv(forecast_file_path, header = F, sep = ",")
...
for(k in 1 :nrow(forecasts_df)){
  ...
  one_line_test_data = as.numeric(text_test_df[finalindex,])
  ...
  for (ii in 1:output_size) {
    converted_value = exp(one_ts_forecasts[ii] + level_value + seasonal_values[ii]) -1
